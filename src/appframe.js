...
var appFrameActions = {
  ...
  createPageInstance: (vars, {className, isUnique, instanceName, title}) => {
    ...
    if (isUnique && prevInstance) {
      if (prevActiveInstance != prevInstance) {
  ...
  return (
    <table>
      <tbody><tr>
        ...
        <td><button onClick={() => {props.closeHook()}}>Close</button></td>
      </tr></tbody>
    </table>
  );
}
    ...
    var elements = Object.keys(pages).map(
        (instName) => {
          ...
          return (
            ...
            >
              <HeaderComponent title={page.title} closeHook={closeHook} />
              <ContainedComponentClass />
            </PageFrame>
          )
        }
      ) // map
