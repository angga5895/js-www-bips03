...
var NetAppVars = {
  ...
  url: 'ws://bahana.ihsansolusi.co.id:5050',
  socket: null,
...
function handleConnectionEvent(vars, isConnected, wsObject) {
  ...
  if (vars.onNetworkConnection) 
    vars.onNetworkConnection(isConnected);
...
const WSConnectionActionF = ContextConnector(NetAppContext, 
  ...
  ['createAndConnect', 'disconnect', 'send']
)(
  (props) => {
...
class WSConnectionAction extends React.Component {
  ...
  notifyActionWS(passedProps) {
    var methodNames = ['createAndConnect', 'disconnect', 'send', 'connectionState'];
    for (var i = 0; i < methodNames.length; ++i)
      this[methodNames[i]] = passedProps[methodNames[i]];
  }
